import { obterUrlImagemCarta } from './api.js';

export async function renderizarWorkspace(deckAtual, moverPrioridade, toggleRole) {
    ...
    for (const [index, item] of deckAtual.entries()) {
...
export async function atualizarPainelEstatisticas(deckAtual) {
    ...
    for (const item of deckAtual) {
        const imgUrl = await obterUrlImagemCarta(item.card.id);
